...
if(isset($_POST['submit_Organization'])){
    ...
    if (empty($_POST['organization'])) {
        header('location:../addjob.php?response1=error&class1=danger&message1=Please fill the Record');
    }
    else{
        $image = $_FILES['image_logo']['tmp_name'];
        $image_name =$_FILES['image_logo']['name'];
        $location = BASE_PATH.'img/logo/';
        $db_path = 'img/logo/';
        ...
        if (strlen($organization) > 10 ) {
            ...
            if (move_uploaded_file($image, $location.$image_name)) {
                $query = mysqli_query($con,"insert into organization(organization_image,organization_name,insert_by) values('$db_path$image_name','$organization','$insert_by')");
                if ($query == 1) {
...
elseif(isset($_POST['submit'])){
    ...
    if(empty($_POST['job_title']) || empty($_POST['content'])  || empty($_POST['issue_date']) || empty($_POST['source'])  ){
        ...
        $location =BASE_PATH.'img/ads_pic/';
        $db_path = 'img/ads_pic/';
        ...
        // echo "insert into job_ads(organization_id,image,job_title,content,issue_date,issue_time,source,insert_by) values('$choose_organization','$db_path$image_name','$job_title','$content','$issue_date','$issue_time','$source','$insert_by')";
        // exit;   

        //$organization_limit = substr($organization, 0,8);
        if (move_uploaded_file($image,$location.$image_name)) {
            $query = mysqli_query($con,"insert into job_ads(organization_id,image,job_title,content,issue_date,issue_time,source,insert_by) values('$choose_organization','$db_path$image_name','$job_title','$content','$issue_date','$issue_time','$source','$insert_by')");
            if ($query == 1) {
